...
import { PlusIcon, Edit2Icon, Trash2Icon, ServerIcon, BotIcon, LoaderIcon } from 'lucide-react';

export function AdminPage() {
  const { getAllERPs, deleteERPConfig, addERPConfig, updateERPConfig, isLoading } = useERPContext();
  ...
  const handleSave = async (config: ERPConfig) => {
    setSaving(true);
    setSaveError(null);
    try {
      if (isAdding) {
        await addERPConfig(config);
      } else {
        await updateERPConfig(config.id, config);
      }
      setIsAdding(false);
      setEditingId(null);
    } catch {
  ...
  const handleDelete = async (erp: ERPConfig) => {
    if (!window.confirm(`Are you sure you want to delete ${erp.name}?`)) return;
    try {
      await deleteERPConfig(erp.id);
    } catch {
    ...
    return (
      <>
        ...
        <IntegrationForm
          ...
          onSubmit={handleSave}
          onCancel={() => { setIsAdding(false); setEditingId(null); setSaveError(null); }}
          saving={saving}
        />
      </>
    );
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-300">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center border-b border-[#E5E7EB] pb-6 gap-4">
        <div>
          <h1 className="text-3xl font-extrabold text-[#1A1F36]">Admin Dashboard</h1>
      ...
      {isLoading ? (
        <div className="flex items-center justify-center py-20 text-[#4B5563]">
          <LoaderIcon className="h-6 w-6 animate-spin mr-3 text-[#5C4EBF]" />
          Loading integrations...
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {erps.map(erp => (
            <div key={erp.id} className="bg-white rounded-xl border border-[#E5E7EB] p-6 shadow-sm flex flex-col hover:border-[#5C4EBF] transition-colors">
              ...
              <div className="space-y-3 pt-4 border-t border-[#E5E7EB]">
                <div className="flex items-center text-sm">
                  ...
                  <span className="text-[#1A1F36] font-bold">Version: {erp.version}</span>
                </div>
                <div className="flex items-center text-sm">
                  <BotIcon className={`h-4 w-4 mr-2 ${erp.agent.copilotEndpoint ? 'text-[#5C4EBF]' : 'text-[#9CA3AF]'}`} />
                  <span className={erp.agent.copilotEndpoint ? 'text-[#5C4EBF] font-bold' : 'text-[#4B5563] font-medium'}>
                    {erp.agent.copilotEndpoint ? 'Copilot Connected' : 'Demo Agent'}
                  </span>
                </div>
              </div>
            </div>
          ...
          {erps.length === 0 && (
            <div className="col-span-full text-center py-12 bg-white rounded-xl border border-[#E5E7EB] border-dashed">
              <p className="text-[#4B5563] font-medium">No integrations found. Add one to get started.</p>
            </div>
          )}
        </div>
      )}
